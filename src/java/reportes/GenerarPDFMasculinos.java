...
@WebServlet("/masculinoPDF")
public class GenerarPDFMasculinos extends HttpServlet {
    @Override
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        response.setContentType("application/pdf");
        response.setHeader("Content-Disposition", "attachment; filename=\"empleados_masculinos.pdf\"");
        ...
        try (OutputStream out = response.getOutputStream()) {
            ...
            try (PdfDocument pdfDoc = new PdfDocument(writer); Document document = new Document(pdfDoc, PageSize.A4)) {
                ...
                document.add(new Paragraph("Lista de Empleados Maculinos").setFontSize(18).setBold());

                // Obtener datos y verificar su carga
                Empleados_adm empModel = new Empleados_adm();
                DefaultTableModel tablaEmpleadosFemeninos = empModel.leerEmpleadosMasculinos();

                if (tablaEmpleadosFemeninos.getRowCount() == 0) {
                    document.add(new Paragraph("No se encontraron datos de empleados Masculinos."));
                } else {
                    // Tabla básica
